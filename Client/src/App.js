...
const App = () => {
	...
	const addTodo = async (todo) => {
	      await axios.post('http://localhost:4001/api/v1/todos', todo);
	...
	const editTodo = async (id, newContent) => {
		...
		await axios.patch(`http://localhost:4001/api/v1/todos/${id}`, newContent);
		
		// setTodos(prevState => [...prevState, newContent]);
